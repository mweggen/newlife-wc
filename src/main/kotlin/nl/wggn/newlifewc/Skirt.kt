...
open class Skirt(type: List<OutfitType>, colour: Colour, override val length: Length, attractive: Int, cute: Int, elegant: Int,
    ...
    companion object {
        fun createWorkSkirt(style: Style, variant: Variant?, colourParam: Colour?, lengthParam: Length?, topType: TopType?, flagsParam: Map<Flag, Boolean>): Skirt {
            val flags = listOf(Flag.SINGULAR)
            ...
            val longDesc = if (length == Length.THIGH) "A short business skirt. Some might say it's too short for the office, but you won't get fired for wearing it. "
            else "A skirt designed for business wear. It's only really suited to a work environment. "

            val attractive = 2 + if (length == Length.THIGH) 1 else 0
            val cute = 0 + if (length == Length.THIGH) -3 else 0
            val elegant = 3 + if (length == Length.THIGH) -2 else if (length == Length.ANKLES) 3 else 0

            val shortDesc = when (length) {
                Length.THIGH -> "short "
                Length.KNEES -> "knee-length "
                else -> "long "
            } + colour.desc + " " + if (length == Length.THIGH) "work skirt" else "pencil suit-skirt"

            val skirt = Skirt(listOf(OutfitType.BUSINESS), colour, length, attractive, cute, elegant, shortDesc, longDesc, 25)
...
object BareLowerBody : Skirt(pantiesOutfitTypes.toList(), Colour.ANY, Length.THIGH, 3, -2, -2, "bare lower body", "", 0) {
    override val detailDesc: String
        get() = "Bare lower body. You can't go out in public like this. "
}
